...
1. **Mark Setup** — configure every exam/class subject, the exam’s percentage-to-grade bands, the pass percentage, and configurable scholarship percentage ranges.
...
- Mark Setup owns subject configuration; the Exams page must not remain a second independent subject editor.
- Scholarship categories are user-defined named ranges. Students outside all configured ranges are `NOT_ELIGIBLE`.
- Grade bands use editable minimum and maximum percentages, with validation for bounds, ordering, overlap, and complete 0–100 coverage.
...
- Mark Setup saves subjects, grade configuration, pass percentage, and scholarship categories through one atomic action.
...
Create `supabase/migrations/0025_marks_management_redesign.sql` and update the baseline `supabase/schema.sql`.

Add `public.exam_mark_configs` with one row per exam:

- `exam_id` primary key and foreign key to `exams` with cascade delete.
- `grade_bands` JSONB containing `{ id, grade, minPercent, maxPercent }` rows.
...
- Validate grade ranges cover 0–100 without overlap and that scholarship ranges do not overlap.
...
- Show the exam-wide grade-band editor with grade, minimum percentage, and maximum percentage.
...
Update the Exams page so subject editing is no longer an independent source of truth. Preserve exam/classes creation, but replace the subject-editing step with a clear callout/link directing the user to Mark Setup after the exam is saved. Existing subject data remains readable by the new setup page.
