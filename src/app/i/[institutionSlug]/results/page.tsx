      ...
      <div className="max-w-[1600px] mx-auto p-6 lg:p-8">
        ...
        <div className="mb-8">
          ...
          <p className={`text-sm mt-1 ${isDark ? "text-zinc-500" : "text-zinc-500"}`}>
            {isBn ? 'আপনার প্রতিষ্ঠানের পরীক্ষার ফলাফল দেখুন' : 'View examination results for your institution'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { icon: Trophy, label: isBn ? 'মোট প্রার্থী' : 'Candidates', value: totalCandidates },
            { icon: Award, label: isBn ? 'পাসের হার' : 'Pass Rate', value: `${totalCandidates > 0 ? Math.round((passed / totalCandidates) * 100) : 0}%` },
            { icon: Award, label: isBn ? 'বৃত্তিপ্রাপ্ত' : 'Scholarship winners', value: scholarshipWinners },
            { icon: Trophy, label: isBn ? 'গড় স্কোর' : 'Avg Score', value: `${avgScore}%` },
          ].map((s) => (
        ...
        <div className={`${card}`}>
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className={`text-xs mt-1 ${isDark ? "text-zinc-500" : "text-zinc-400"}`}>{isBn ? 'পরীক্ষা প্রকাশের পর ফলাফল এখানে দেখা যাবে।' : 'Results will appear here once exams are published.'}</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
