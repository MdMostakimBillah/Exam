          ...
          <div className={`px-5 py-4 border-b ${isDark ? "border-white/[0.06]" : "border-zinc-100"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ClipboardList className={`h-4 w-4 ${isDark ? "text-zinc-500" : "text-zinc-400"}`} />
            ...
            <Table>
              <TableHeader>
                <TableRow className={isDark ? 'border-white/[0.04] hover:bg-transparent' : 'border-zinc-100 hover:bg-transparent'}>
                  <TableHead className={`text-[10px] font-medium uppercase tracking-wider ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>{isBn ? 'আবেদন আইডি' : 'Application ID'}</TableHead>
                  ...
                  <TableHead className={`text-[10px] font-medium uppercase tracking-wider hidden lg:table-cell ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>{isBn ? 'পরীক্ষা' : 'Exam'}</TableHead>
                  ...
                  <TableHead className={`text-[10px] font-medium uppercase tracking-wider ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>{isBn ? 'পেমেন্ট' : 'Payment'}</TableHead>
              ...
              <TableBody>
                {filtered.slice(0, 20).map(reg => (
                  <TableRow key={reg.id} className={`${isDark ? 'border-white/[0.04] hover:bg-white/[0.02]' : 'border-zinc-100 hover:bg-zinc-50/50'}`}>
                    ...
                    <TableCell className={`text-[11px] hidden md:table-cell ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>{reg.institutionName}</TableCell>
                    <TableCell className={`text-[11px] hidden lg:table-cell ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>{reg.examName}</TableCell>
                    <TableCell className={`text-[11px] ${isDark ? "text-zinc-500" : "text-zinc-400"}`}>{formatDate(reg.createdAt)}</TableCell>
