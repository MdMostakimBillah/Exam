...
export default function NotificationsPage() {
  ...
  const colorMap = { success: 'text-emerald-400 bg-emerald-500/10', warning: 'text-amber-400 bg-amber-500/10', info: 'text-blue-400 bg-blue-500/10', error: 'text-red-400 bg-red-500/10' };
          ...
          {notifications.length === 0 ? (
            ...
            <div className="divide-y divide-white/[0.04]">
              {notifications.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(n => {
                ...
                return (
                  <div key={n.id} className={`flex items-start gap-3 p-4 ${!n.read ? 'bg-white/[0.02]' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-zinc-700 mt-1">{new Date(n.createdAt).toLocaleString()}</p>
                    </div>
