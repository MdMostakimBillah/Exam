...
import { FileText, Search, Plus, Edit, Trash2, Calendar, Users, CreditCard, FileDown, ArrowLeft, ArrowRight, X, RefreshCw, Settings2 } from "lucide-react";
...
export default function ExamsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingExam) {
        const { subjects: _subjects, ...metadata } = payload;
        await updateExamMutation.mutateAsync({ id: editingExam.id, data: metadata });
          ...
          <div className={`px-4 py-2 border-b flex items-center gap-2 ${isDark ? 'border-white/[0.06]' : 'border-zinc-200'}`}>
              {[
                ...
                <div key={step.num} className="flex items-center gap-2 flex-1">
                  ...
                  {i < 1 && <div className={cn("flex-1 h-px mx-2", examStep > step.num ? "bg-green-500" : isDark ? "bg-white/[0.08]" : "bg-zinc-200")} />}
                </div>
              ))}
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            ...
            {examStep === 2 && (
              <div className="space-y-5">
                ...
                <div className={`rounded-md border p-5 ${isDark ? 'border-brand-accent/20 bg-brand-accent/5' : 'border-brand-accent/20 bg-brand-accent-soft'}`}>
